            ...
            Inner::Tty { events, .. } => loop {
                ...
                match event.map_err(Error::from)? {
                    crossterm::event::Event::Key(key) => {
                        // A terminal with the kitty protocol on reports
                        // releases and repeats too; only a press is input.
                    ...
                    crossterm::event::Event::Resize(cols, rows) => {
                        return Ok(Event::Resize(Size::new(cols, rows)));
                    }
...
// depth: crossterm key events.

fn from_crossterm(event: crossterm::event::KeyEvent) -> Option<KeyPress> {
